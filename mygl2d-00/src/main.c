...
//	updateAnimation(&spinAnim,deltaTime);
	updateAnimation(&manAnim,deltaTime);
}
...
//	drawTargaX(spin,64,16,spinFrames[spinAnim.currentFrame],mouse->x,mouse->y,MYGL2D_FLIP_NONE,0,0,0,1);
	...
	drawTargaX(man,272,68,manFrames[manAnim.currentFrame],mouse->x,mouse->y,MYGL2D_FLIP_NONE,0,0,0,1);
...
int main(int argc, char** argv) {
	...
	man=loadTarga("images/man.tga");

	spinAnim=(Animation){spinFrames,4,0,0.1f,0.0f};
